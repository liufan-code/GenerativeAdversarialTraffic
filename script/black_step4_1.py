...
model = load_model("E://WorkPlace/model/cnn_substitute.h5")
...
adv_x = momentum_iterative_method(logits_model, X_test_cnn, **mim_params)
adv_pred = model.predict(adv_x)

pd_adv_x = DataFrame(tf.reshape(adv_x, (X_test.shape[0], X_test.shape[1])).numpy())
pd_adv_x.to_csv("E://WorkPlace/black_csv/adv_cnn_mim.csv")
...
adv_x = carlini_wagner_l2(logits_model, X_test_cnn, **cw_params)
adv_pred = model.predict(adv_x)

pd_adv_x = DataFrame(tf.reshape(adv_x, (X_test.shape[0], X_test.shape[1])).numpy())
pd_adv_x.to_csv("E://WorkPlace/black_csv/adv_cnn_cw.csv")
